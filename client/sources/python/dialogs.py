...
class BaseDialog(QDialog):
    ...
    def create_input_field(
        ...
        label = QLabel(russian_label_text)
        ...
        if items is not None:
            ...
            if isinstance(items[0], dict):
                ...
                for item in items:
                    input_field.addItem(str(item), item)
        else:
...
class AddDialog(BaseDialog):
    ...
    def __init__(self, columns, table_type, data_source, parent=None, parent_app=None):
        ...
        self.setStyleSheet(WINDOW_STYLES["add"])
...
class AddTempDialog(BaseDialog):
    ...
    def __init__(self, data, parent=None):
        super().__init__("Добавить пользователя", parent)
        self.setStyleSheet(WINDOW_STYLES["add_temp"])
        self.data = data
        self.row_index = None
        layout = QVBoxLayout()
...
class DeleteDialog(BaseDialog):
    ...
    def __init__(self, data, parent=None):
        ...
        self.setStyleSheet(WINDOW_STYLES["delete"])
        self.data = data
        self.row_index = None
        layout = QVBoxLayout()
...
class UpdateDialog(BaseDialog):
    ...
    def __init__(self, columns, table_type, data_source, parent=None, parent_app=None): 
        ...
        self.inputs = {}
        self.parent_app = parent_app
        self.data = self.data_source.get("data", {}).get(TABLES[table_type].get("data_key", table_type), [])
        layout = QVBoxLayout()
...
class SelectSemesterDialog(BaseDialog):
    ...
    def __init__(self, semesters, parent=None):
        ...
        self.semester_combo.addItem("Все семестры")
